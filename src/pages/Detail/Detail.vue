      ...
      <div class="mainCon">
        ...
        <div class="InfoWrap">
          <div class="goodsDetail">
            <h3 class="InfoName">{{ skuInfo.skiName }}</h3>
...
export default {
  ...
  methods: {
    changeActive(saleAttrValue, attr) {
      saleAttrValue.forEach((item) => {
        item.isChecked = '0'
      })
      attr.isChecked = '1'
    },
    checkSkuNum(event) {
      let inputVaule = event.target.value * 1
      if (isNaN(inputVaule) || inputVaule < 1) {
        this.skuNum = 1
      } else {
        this.skuNum = parseInt(inputVaule)
      }
    },
    async addOrUpdateCart() {
      //派发action:携带的载荷，分别商品的id、商品个数
      //思考底下的这行代码实质做了一个什么事情?
      //实质就是调用了小仓库里面相应的这个函数->addOrUpdateCart,声明部分加上async,这个函数执行的结构一定是Promise
