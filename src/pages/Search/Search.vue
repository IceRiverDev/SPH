  ...
  <div>
    ...
    <div class="main">
      <div class="py-container">
        ...
        <!--selector-->
        <SearchSelector @tradeMarkInfo="getTradeMark" @attrInfo="getAttrAndAttrValue"/>
...
export default {
  ...
  methods: {
    ...
    changeOrder(flag) {
      ...
      let originalFlag = originalOrder.split(':')[0]
      let originalSort = originalOrder.split(':')[1]
      let newOrder = ''
      if (flag === originalFlag) {
        if (originalSort == 'asc') {
          newOrder = `${originalFlag}:desc`
        } else if (originalSort == 'desc') {
          newOrder = `${originalFlag}:asc`
        }
      } else {
        newOrder = `${flag}:desc`
      }
